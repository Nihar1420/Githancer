...
Githancer assigns pre-scheduled timestamps to your commits. When you run
`timeline commit`, the commit is stamped with the next timestamp from your
project queue — not the current time.

Timestamps are generated by the scheduling mode you chose when creating the
project (linear, random, sprint, etc.).

## Acceptable use

Use Githancer for work you actually did. The timestamps it applies should
correspond to when the underlying work was genuinely performed — for example,
importing commits from private repositories, or committing work that was
written offline over a period of time.

Do not use Githancer to represent work that was not done, or to misrepresent
your development history to anyone evaluating your work professionally.
